...
const { bilder, seo, kontakt } = restaurang;

/* ---------------------------------------------------------------------------
   Startsidans sektioner. Vilka som visas och i vilken ordning styrs av
   `startsidaSektioner` i kundens config - koden här behöver aldrig ändras
   för att stuva om en kunds startsida.
--------------------------------------------------------------------------- */
  ...
  return (
    ...
    >
      <div className="grid gap-12 lg:grid-cols-[minmax(0,20rem)_1fr] lg:gap-16">
        ...
        {restaurang.sektioner.karta && <Karta />}
      </div>
    </Sektion>
  );
}
...
export default function Startsida() {
  return (
    <>
      <Hero />
      {restaurang.startsidaSektioner.map((namn) => {
        const Komponent = SEKTIONER[namn];
        return <Komponent key={namn} />;
      })}
    </>
  );
}
